...
class RcBeamDsgnWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.drawornot == 'yes':
            # 畫梁與柱
            p1x = int(self.wsize / 2 - self.L / 2 - self.colw / 2)
            p2x = int(self.wsize / 2 - self.L / 2 + self.colw / 2)
            p3x = int(self.wsize / 2 + self.L / 2 - self.colw / 2)
            p1y = int(self.hsize / 2 - self.colh - self.D / 2)
            p2y = int(self.hsize / 2 - self.D / 2)
            ...
            self.qpainter.drawLine(
                int(p2x - self.colw / 2),
                int(p2y + 7 * self.factor),
                int(p3x + self.colw / 2),
                int(p2y + 7 * self.factor)
            )
            self.qpainter.drawLine(
                int(p2x - self.colw / 2),
                int(p2y + self.D - 10 * self.factor),
                int(p3x + self.colw / 2),
                int(p2y + self.D - 7 * self.factor)
            )
            ...
            for i in range(6):
                if self.dsgn_barnum[i] <= maxnumperrow:
                    self.qpainter.drawText(
                        ...
                        int(20),
                        Align[xnum[i]],
                        str(self.dsgn_barnum[i]) + ' - ' + self.choose_bar
                    )
                else:
                    double_layer = [str(maxnumperrow), str(self.dsgn_barnum[i] - maxnumperrow)]
                    self.qpainter.drawText(
                        int(inputx[xnum[i]]),
                        int(inputy[ynum[i]] + adjusty[ynum[i]]),
                        int(100),
                        int(30),
                        Align[xnum[i]],
                        double_layer[ynum[i]] + ' - ' + self.choose_bar + '\n' +
                        double_layer[1 - ynum[i]] + ' - ' + self.choose_bar
                    )                                                                         
            ...
            self.qpainter.drawText(
                ...
                int(10),
                Qt.AlignLeft,
                str(self.choose_stirrup_num[0]) + ' - ' + self.choose_stirrup[0] + ' @' + str(self.s_dsgn_all[0])
            )
            self.qpainter.drawText(
                int(self.wsize / 2 - 50),
                int(p2y + self.D / 2 - 5),
                ...
                int(10),
                Qt.AlignCenter,
                str(self.choose_stirrup_num[1]) + ' - ' + self.choose_stirrup[1] + ' @' + str(self.s_dsgn_all[1])
            )
            self.qpainter.drawText(
                int(p3x - 0.05 * self.L - 100),
                int(p2y + self.D / 2 - 5),
                int(100),
                int(10),
                Qt.AlignRight,
