...
class RcTBeamWidget(QtWidgets.QWidget):
    ...
    def paintEvent(self, event):
        ...
        if self.drawornot == 'yes':
            # Draw 斷面
            self.qpainter.setBrush(QColor(230, 230, 230))
            self.qpainter.drawRect(QRect(int(0 + self.x_offset), int(0), int(self.B), int(self.D)))
            self.qpainter.drawRect(QRect(int(0), int(0), int(self.x_offset), int(self.hf)))
            if self.BeamCondition == '內梁':
                self.qpainter.drawRect(QRect(int(self.B + self.x_offset), int(0), int(self.x_offset), int(self.hf)))

            # Draw 剪力筋
            qpen = QPen(Qt.red, 1.5, Qt.SolidLine)
            self.qpainter.setPen(qpen)
            self.qpainter.drawRoundedRect(
                int(self.str_start[0]),
                int(self.str_start[1]),
                int(self.str_height),
                int(self.str_width),
                int(self.arcd / 2),
                int(self.arcd / 2)
            )
            self.qpainter.drawArc(
                int(self.str_start[0]),
                int(self.str_start[1]),
                int(self.arcd),
                int(self.arcd),
            ...
            self.qpainter.drawLine(
                int(self.cuvre_start[0]),
                int(self.cuvre_start[1]),
                int(self.cuvre_start[0] + self.cuvre_length / 2**0.5),
                int(self.cuvre_start[1] + self.cuvre_length / 2**0.5)
            )
            self.qpainter.drawLine(
                int(self.cuvre_start[0] - self.arcd / 2**0.5),
                int(self.cuvre_start[1] + self.arcd / 2**0.5),
                int(self.cuvre_start[0] + self.cuvre_length / 2**0.5 - self.arcd / 2**0.5),
                int(self.cuvre_start[1] + self.cuvre_length / 2**0.5 + self.arcd / 2**0.5)
            )
            ...
            for i in range(2):
                for j in range(self.BarNum[i]):
                    if j < self.BarAllowabelNumPerRow[i]:
                        ...
                        if (j - self.BarAllowabelNumPerRow[i]) % 2 == 0:
                            self.qpainter.drawEllipse(
                                int(self.xstart + 0.5 * (j - self.BarAllowabelNumPerRow[i]) * self.db_h[i]),
                                int(self.ystart[i] + self.cleardb_v[i]),
                                int(self.bard[i]),
                                int(self.bard[i])
                            )
                        else:
                            self.qpainter.drawEllipse(
                                int(self.B + 2 * self.x_offset - self.xstart - self.bard[i] - 0.5 * (j - self.BarAllowabelNumPerRow[i] - 1) * self.db_h[i]),
                                int(self.ystart[i] + self.cleardb_v[i]),
                                int(self.bard[i]),
                                int(self.bard[i])
                            )
